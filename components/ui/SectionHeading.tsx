...
export const SectionHeading: React.FC<SectionHeadingProps> = ({
  ...
  return (
    <div className={`text-center max-w-3xl mx-auto ${SECTION_HEADING_GAP} ${className}`}>
      <span
        className={`inline-block text-[11px] sm:text-xs font-bold uppercase tracking-[0.12em] px-3.5 py-1.5 rounded-full border ${
          inverse
      ...
      <h2
        className={`text-2xl sm:text-3xl lg:text-[2.125rem] font-extrabold tracking-tight mt-4 leading-[1.2] ${
      ...
      {description && (
        <p className={`text-sm sm:text-[15px] leading-relaxed mt-3.5 ${inverse ? "text-slate-300" : "text-slate-600"}`}>
          {description}
        </p>
      )}
    </div>
