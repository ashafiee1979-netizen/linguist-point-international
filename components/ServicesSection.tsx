      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
          {PLANS.map((plan) => (
            <div
              key={plan.name}
              className={`relative flex flex-col h-full rounded-2xl p-6 sm:p-7 transition-all ${
                plan.featured
                  ? "bg-white border-2 border-[#173d40] shadow-xl lg:-mt-2"
                  : "bg-white border border-slate-200 shadow-sm hover:border-teal-300 hover:shadow-md"
              }`}
            >
              {plan.badge && (
                <span className="absolute -top-3 left-6 bg-[#f59e0b] text-slate-950 text-[11px] font-extrabold uppercase tracking-wider px-3 py-1 rounded-full shadow-sm">
                  {plan.badge}
                </span>
              )}

              <h3 className={`text-base font-extrabold text-slate-900 ${plan.badge ? "mt-2" : ""}`}>
                {plan.name}
              </h3>
              <p className="text-xs text-slate-600 leading-relaxed mt-2">{plan.blurb}</p>

              <div className="mt-5 pb-5 border-b border-slate-100">
                <div className="flex items-baseline gap-2 flex-wrap">
                  {plan.wasPrice && (
                    <span className="text-base font-bold text-slate-400 line-through">{plan.wasPrice}</span>
                  )}
                  <span className="text-3xl font-extrabold text-[#173d40]">{plan.price}</span>
                </div>
              ...
              <ul className="space-y-2.5 py-5 flex-grow">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-start gap-2 text-xs text-slate-700 leading-snug">
                    <Check className="w-3.5 h-3.5 text-emerald-600 flex-shrink-0 mt-0.5" strokeWidth={2.75} />
                    <span>{feature}</span>
                  </li>
                ))}
              </ul>
              ...
              {plan.serviceType ? (
                <button
                  onClick={() => onOpenOrder(plan.serviceType as ServiceType)}
                  className={`w-full font-bold text-xs px-5 py-3 rounded-xl transition-all flex items-center justify-center gap-2 ${
                    plan.featured
                      ? "bg-[#173d40] hover:bg-[#123032] text-white shadow-md hover:shadow-lg active:scale-98"
                      : "bg-white border-2 border-[#173d40] text-[#173d40] hover:bg-[#173d40] hover:text-white"
                  }`}
                >
                  {plan.cta}
                ...
                <a
                  ...
                  className="w-full font-bold text-xs px-5 py-3 rounded-xl transition-all flex items-center justify-center gap-2 bg-white border-2 border-[#173d40] text-[#173d40] hover:bg-[#173d40] hover:text-white"
                >
                  {plan.cta}
                  <ArrowRight className="w-3.5 h-3.5" />
                </a>
