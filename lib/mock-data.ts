...
export function calculateOrderPrice(inputs: CalculationInputs): CalculationOutputs {
  const pricePerPage = 24.95;
  const rushFee = 14.95;
  const notarizationFee = 19.95;
  const shippingFee = 12.50;

  const base = inputs.pageCount * pricePerPage;
  ...
  const total = base + rush + notarization + shipping;
  const deliveryHours = inputs.isRush12Hour ? 12 : 24;

  return {
    basePrice: parseFloat(base.toFixed(2)),
    rushPrice: parseFloat(rush.toFixed(2)),
    notarizationPrice: parseFloat(notarization.toFixed(2)),
    shippingPrice: parseFloat(shipping.toFixed(2)),
    totalAmount: parseFloat(total.toFixed(2)),
    estimatedDeliveryHours: deliveryHours
  };
}
